...
@router.post("/create")
async def create_task(create_data: CreateTaskSchema):
...
@router.get("/{task_id}")
async def get_task_by_id(task_id: UUID) -> ResponseTaskSchema:
...
async def update_task(task_id: UUID, update_data: UpdateTaskSchema) -> ResponseTaskSchema:
    is_task = await TaskRepository.get_one_or_none(id=task_id)
    if is_task is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
